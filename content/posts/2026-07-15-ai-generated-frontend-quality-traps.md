...
但"改"的工作量到底有多大？HN 上有个评论一针见血：
> **"The gap between 'acceptable' and 'decent' has thus been widening... The end result is more crunch and a sharp decline in software quality."** — *m132, Hacker News*

这正是本文要解决的问题。219 分、136 条评论的热帖 *"Slightly reducing the sloppiness of AI generated front end"* 证明了这是行业普遍共识：AI 生成的前端代码"看着还行"，但离"生产可用"有一道系统性的鸿沟。不是零星的小毛病，而是**一组可以预测、可以分类、可以自动检测的质量问题**。
...
研究数据印证了这一点。Guriță & Vatavu 在 2025 年 ACM WWW 论文中定量分析了 AI 生成 UI 的 WCAG 符合率，发现**超过 52% 的 UI 存在文本对比度违规**，整体严重度评分 M=0.47（0-4 分制）。更值得警惕的是一个反直觉的发现：**在提示词中明确要求"关注无障碍"的 AI 生成结果，合规率反而下降**（M=0.54 vs M=0.39）——因为 AI 在"刻意使用颜色合规"上操作过度，引入了更多违规。

这与 WebAIM Million 2024 的行业基线数据形成对照：96.3% 的全球 Top 100 万首页存在可检测的 WCAG 失败，平均每页 56.8 个错误。AI 没有制造无障碍危机，但它放大了现有问题——因为 AI 生成的代码正在以前所未有的速度进入生产环境。
...
- **[axe-core](https://github.com/dequelabs/axe-core)**（Deque Labs，v4.12.1，2026 年 6 月更新）：行业标准，新增 aria-tab-name 规则
- **`@axe-core/playwright`**：在 CI 中自动扫描每个页面
- **Lighthouse CI**：检查 a11y 分数是否低于阈值（建议：90 分以下不通过）

> ⚠️ 注意：axe-core 能自动化检测大量问题，但**真正合规（尤其是避免法律风险）需要 E2E 测试 + 人工判断**——键盘焦点陷阱、焦点恢复、`alt` 文本语义正确性等问题，自动化工具无法覆盖。
...
- HN 讨论："Is AI causing a repeat of frontend's lost decade?" — https://news.ycombinator.com/item?id=48321631
- Guriță & Vatavu (2025) "Insights and Implications of Evaluating Accessibility Compliance in AI-Generated Web Interfaces" — ACM WWW Companion '25, DOI: 10.1145/3701716.3715552
- Mowar et al. (2025) "CodeA11y: Making AI Coding Assistants Useful for Accessible Web Development" — CHI 2025, arXiv: 2502.10884
- WebAIM Million 2024 年度无障碍报告 — https://webaim.org/projects/million/
